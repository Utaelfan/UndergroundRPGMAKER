 ...
 * - Checks if the current [CGMZ] difficulty is in the given difficulty list
 * - Please note that events do not refresh until an action occurs that
 *   causes events to check their pages, such as switch or variable changes.
 *
 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version adds an integration with [CGMZ] Difficulty so
 * you can now have different event pages activate based on the player's
 * current difficulty selected in that plugin.
 *
 * Version 1.3.0
...
CGMZ.Versions["Unlimited Event Conditions"] = "1.3.0";
...
Game_Event.prototype.CGMZ_EventConditions_meetsExtraCondition = function(condition) {
	...
	const paramsCapital = condition.split(" ");
	switch(params[0]) {
		...
		case "difficulty:": return this.CGMZ_EventConditions_checkDifficulty(paramsCapital[1]);
	}
...
//-----------------------------------------------------------------------------
// Check difficulty extra condition
//-----------------------------------------------------------------------------
Game_Event.prototype.CGMZ_EventConditions_checkDifficulty = function(difficultyList) {
	// Expected Format: Difficulty: [difficulty1,difficulty2]
	if(!difficultyList || !Imported.CGMZ_Difficulty) return true;
	const list = difficultyList.split(',');
	if(list.length === 0) return true;
	const currentDifficulty = $cgmz.getDifficulty();
	return list.includes(currentDifficulty);
};
//-----------------------------------------------------------------------------
